...
func Init() {
	...
	log.GetLogger().WithField("config", viper.AllSettings()).Info("viper loaded configuration")

	if err := viper.Unmarshal(config.Config); err != nil {
		panic(err)
	}

	log.GetLogger().WithField("config", config.Config).Info("unmarshalled configuration")
	...
	log.GetLogger().Logger.SetLevel(logLevel)
	log.GetLogger().Debugf("log level set: %s", config.Config.Log.GetLogLevel())

	log.GetLogger().Infof("database name: %s", databaseName)
	...
	for _, object := range objects {
		...
		if err != nil {
			log.GetLogger().WithError(err).Error("migrate error")
		}
	}
}
